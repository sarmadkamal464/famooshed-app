...
class DocumentPage extends GetView<DocumentController> {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<DocumentController>(
      builder: (DocumentController documentController) {
        return Scaffold(
          ...
          body: SafeArea(
            child: SingleChildScrollView(
              child: Column(
                children: [
                  Html(data: documentController.privacyText),
                  SizedBox(
                    height: getProportionateScreenHeight(10),
                  ),
                  Html(data: documentController.termsText),
                  SizedBox(
                    height: getProportionateScreenHeight(10),
                  ),
                  Html(data: documentController.refundText),
                ],
              ),
            ),
          ),
